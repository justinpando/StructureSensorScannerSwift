...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject : AnyObject]?) -> Bool {
        ...
        if UIApplication.sharedApplication().applicationState == .Background {
            print("iOS launched %@ in the background.  This app is not designed to be launched in the background so it will exit peacefully.", NSBundle.mainBundle().objectForInfoDictionaryKey("CFBundleDisplayName"))
            exit(0)
        }
        self.window = UIWindow(frame: UIScreen.mainScreen().bounds)
        // Override point for customization after application launch.
        if UIDevice.currentDevice().userInterfaceIdiom == .Phone {
            self.viewController = ViewController(nibName: "ViewController_iPhone", bundle: nil)
        }
        else {
            self.viewController = ViewController(nibName: "ViewController_iPad", bundle: nil)
        }
